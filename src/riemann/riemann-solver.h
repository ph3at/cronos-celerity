...
static inline void computeFluxes(PhysValues& fields, const FieldStruct& reconstruction, const unsigned int face) {
    unsigned dir = face / 2;
    ...
    for (unsigned fluxDir = FieldNames::VELOCITY_X; fluxDir <= FieldNames::VELOCITY_Z; fluxDir++) {
        if (fluxDir == dir + FieldNames::VELOCITY_X) {
            fields.fluxes[fluxDir] = fields.conservatives[fluxDir] * reconstruction[fluxDir] + fields.thermalPressure;
        } else {
            fields.fluxes[fluxDir] = fields.conservatives[fluxDir] * reconstruction[FieldNames::VELOCITY_X + dir];
        }
    }
...
characteristicVelocity(const PhysValues& physValsLeft, const PhysValues& physValsRight, const FieldStruct& reconstLeft,
                       const FieldStruct& reconstRight, const double gamma, const unsigned dir) {
    ...
    double soundVelocityLeft =
        std::sqrt(gamma * physValsLeft.thermalPressure / physValsLeft.conservatives[FieldNames::DENSITY]);
    double soundVelocityRight =
        std::sqrt(gamma * physValsRight.thermalPressure / physValsRight.conservatives[FieldNames::DENSITY]);

    double characVelPlus =
        std::max(std::max(soundVelocityLeft + flowVelocityLeft, soundVelocityRight + flowVelocityRight), 0.0);
    double characVelMinus =
        std::max(std::max(soundVelocityLeft - flowVelocityLeft, soundVelocityRight - flowVelocityRight), 0.0);

    return std::make_pair(characVelPlus, characVelMinus);
}

static inline FieldStruct numericalFluxOutsideRiemannFan(const PhysValues& physValues) {
...
static inline std::tuple<unsigned, unsigned, unsigned> directionalIndices(unsigned dir) {
    switch (dir) {
        case Direction::DirX: {
            return std::make_tuple(FieldNames::VELOCITY_X, FieldNames::VELOCITY_Y, FieldNames::VELOCITY_Z);
        }
        case Direction::DirY: {
            return std::make_tuple(FieldNames::VELOCITY_Y, FieldNames::VELOCITY_Z, FieldNames::VELOCITY_X);
        }
        case Direction::DirZ: {
            return std::make_tuple(FieldNames::VELOCITY_Z, FieldNames::VELOCITY_X, FieldNames::VELOCITY_Y);
        }
        default: {
...
static inline FieldStruct computeStarValues(const PhysValues& physValues, const FieldStruct& reconstruction,
    ...
    starValues[FieldNames::DENSITY] = physValues.conservatives[FieldNames::DENSITY] * relativeSignalVelocity /
    ...
    starValues[velocityPerpendicular1] = starValues[FieldNames::DENSITY] * reconstruction[velocityPerpendicular1];
    starValues[velocityPerpendicular2] = starValues[FieldNames::DENSITY] * reconstruction[velocityPerpendicular2];

    starValues[FieldNames::THERMAL_ENERGY] =
        starValues[FieldNames::DENSITY] *
        (physValues.conservatives[FieldNames::THERMAL_ENERGY] / physValues.conservatives[FieldNames::DENSITY] +
         (intermediateWaveSpeed - reconstruction[velocityParallel]) *
             (intermediateWaveSpeed +
              physValues.thermalPressure / (physValues.conservatives[FieldNames::DENSITY] * relativeSignalVelocity)));

    return starValues;
}

static inline FieldStruct computeNumericalFlux(const PhysValues& physValues, const FieldStruct& reconstruction,
                                               const double characteristicVelocity, const double relativeSignalVelocity,
                                               const double intermediateWaveSpeed, const unsigned velocityParallel,
                                               const unsigned velocityPerpendicular1,
                                               const unsigned velocityPerpendicular2) {
    FieldStruct numericalFluxes = {};
    FieldStruct starValues =
        computeStarValues(physValues, reconstruction, characteristicVelocity, relativeSignalVelocity,
                          intermediateWaveSpeed, velocityParallel, velocityPerpendicular1, velocityPerpendicular2);
    for (unsigned field = 0; field < NUM_PHYSICAL_FIELDS; field++) {
        numericalFluxes[field] =
            physValues.fluxes[field] + characteristicVelocity * (starValues[field] - physValues.conservatives[field]);
    }
    return numericalFluxes;
}

static inline FieldStruct numericalFluxInsideRiemannFan(const std::pair<double, double>& characteristicVelocities,
    ...
    std::tie(velocityParallel, velocityPerpendicular1, velocityPerpendicular2) = directionalIndices(dir);
    ...
    double intermediateWaveSpeed =
        (thermalPressurePlus - thermalPressureMinus + massFlowRateMinus * relativeSignalVelocityMinus -
         massFlowRatePlus * relativeSignalVelocityPlus) /
        (densityMinus * relativeSignalVelocityMinus - densityPlus * relativeSignalVelocityPlus + HLLCSOLVER_HYDRO_VEPS);

    if (intermediateWaveSpeed >= 0.0) {
        return computeNumericalFlux(physValsRight, reconstructionRight, characteristicVelocityMinus,
                                    relativeSignalVelocityMinus, intermediateWaveSpeed, velocityParallel,
                                    velocityPerpendicular1, velocityPerpendicular2);
    } else {
        return computeNumericalFlux(physValsLeft, reconstructionLeft, characteristicVelocityPlus,
                                    relativeSignalVelocityPlus, intermediateWaveSpeed, velocityParallel,
                                    velocityPerpendicular1, velocityPerpendicular2);
    }
}

static inline FieldStruct numericalFlux(const std::pair<double, double>& characteristicVelocities,
                                        const PhysValues& physValsLeft, const PhysValues& physValsRight,
                                        const FieldStruct& reconstructionLeft, const FieldStruct& reconstructionRight,
                                        const unsigned dir) {
    ...
    if (characteristicVelocities.second <= 0) {
        ...
        return numericalFluxInsideRiemannFan(characteristicVelocities, physValsLeft, physValsRight, reconstructionLeft,
                                             reconstructionRight, dir);
    }
}
